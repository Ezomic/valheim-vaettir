...
namespace Grove
{
    ...
    internal static class ObjMesh
    {
        public static ModelData Load(string path)
        {
            if (!File.Exists(path))
            {
                GrovePlugin.Log.LogWarning("No model at " + path);
                return null;
            }
            ...
            {
                GrovePlugin.Log.LogError("Could not read " + path + ": " + e.Message);
                return null;
            }
        }
        ...
        private static ModelData Parse(string[] lines, string name)
        {
            ...
            {
                GrovePlugin.Log.LogError("Model " + name + " has no geometry.");
                return null;
            }
            ...
            if (outPositions.Count > 65535)
            {
                ...
                GrovePlugin.Log.LogInfo(name + " needs 32 bit indices ("
            ...
            GrovePlugin.Log.LogInfo(string.Format("Loaded {0}: {1} verts, {2} tris, parts [{3}]",
