...
namespace Furrow
{
    ...
    internal static class Layout
    {
        ...
        /// <summary>
        /// A filled square around the centre, nearest cells first, so a partial count
        /// still comes out compact rather than as one long arm. The centre cell is the
        /// game's own seed and is skipped.
        /// </summary>
        private static void Square(List<Vector3> offsets, int extras, float step)
        {
            if (extras <= 0) return;

            // Enough rings to hold the count: ring r holds the cells whose Chebyshev
            // distance is exactly r, which is 8r cells.
            var cells = new List<Vector3>();
            for (var ring = 1; cells.Count < extras && ring < 16; ring++)
            {
                for (var x = -ring; x <= ring; x++)
                for (var z = -ring; z <= ring; z++)
                {
                    if (Mathf.Max(Mathf.Abs(x), Mathf.Abs(z)) != ring) continue;
                    cells.Add(new Vector3(x * step, 0f, z * step));
                }
            }

            // Nearest first, so five seeds make a plus-sign rather than a corner.
            cells.Sort((a, b) => a.sqrMagnitude.CompareTo(b.sqrMagnitude));
            for (var i = 0; i < extras && i < cells.Count; i++)
                offsets.Add(cells[i]);
        }
