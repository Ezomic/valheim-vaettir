    ...
    internal static class Sowing
    {
        ...
        public static void HandleKeys(Player player)
        {
            ...
            if (Keys.Pressed(FurrowConfig.ShapeKey.Value))
            {
                ...
                // The grid is the level-10 shape: a rank of seeds is a hand skill, a
                // field laid out at once is what the levels are for. Skipped rather
                // than refused, so the key still cycles somewhere useful.
                if (next == SowShape.Grid
                    && player.GetSkillFactor(Skills.SkillType.Farming) * 100f
                       < FurrowConfig.GridLevel.Value)
                {
                    player.Message(MessageHud.MessageType.TopLeft,
                        "Furrow: grid needs Farming " + FurrowConfig.GridLevel.Value);
                    next = SowShape.Row;
                }
        ...
        private static void SowTheRest(Player __instance, Piece piece, bool __result)
        {
            ...
            // The gate again at sowing time: the shape can be grid from an earlier,
            // higher-skilled character on this machine, and the skill can have been
            // death-drained since it was chosen.
            var shape = Shape == SowShape.Grid && level < FurrowConfig.GridLevel.Value
                ? SowShape.Row
                : Shape;
