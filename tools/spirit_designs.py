...
def ribcage(centre_z, height, radius, staves=5, stave=0.055):
    ...
    box((radius * 1.5, radius * 1.5, height * 0.66), (0.0, 0.0, centre_z), "core")
    for i in range(staves):
        rad = math.radians(360.0 / staves * i + 18.0)
        box((stave, stave, height), (math.cos(rad) * radius, math.sin(rad) * radius,
                                     centre_z), "bark")
...
def roots(at, spread, count=5, length=0.34, thick=0.075):
    ...
    a silhouette can have. Roots cost four extra parts and remove the reading.
    """
    x, y, z = at
    for i in range(count):
        rad = math.radians(360.0 / count * i + 25.0)
        box((thick, thick, length),
            (x + math.cos(rad) * spread, y + math.sin(rad) * spread, z),
            "bark", rot_x=math.sin(rad) * 58.0, rot_y=-math.cos(rad) * 58.0)
...
def warden():
    ...
    """
    for x in (-0.20, 0.20):
        roots((x, 0.0, 0.16), 0.13)
        taper(0.13, 0.10, 0.52, (x, 0.0, 0.42), "bark")

    taper(0.30, 0.26, 0.20, (0.0, 0.0, 0.74), "bark")
    ribcage(1.06, 0.56, 0.22, staves=5)

    taper(0.26, 0.44, 0.26, (0.0, 0.0, 1.42), "bark")
    taper(0.46, 0.30, 0.18, (0.0, 0.0, 1.56), "moss", sides=8)

    # Long arms, hung from the outside of the shoulder mass so they clear the cage.
    for x, tilt in ((-0.40, 9), (0.40, -9)):
        box((0.09, 0.09, 0.66), (x, 0.04, 1.16), "bark", rot_y=tilt)
        box((0.08, 0.08, 0.56), (x * 1.16, 0.06, 0.62), "bark", rot_y=-tilt * 0.5)

    taper(0.16, 0.13, 0.22, (0.0, -0.02, 1.72), "bark")
    box((0.12, 0.04, 0.05), (0.0, -0.13, 1.74), "core")

    for x in (-0.12, 0.12):
        box((0.06, 0.06, 0.44), (x, 0.0, 1.98), "bark", rot_y=-30 if x < 0 else 30)
        box((0.05, 0.05, 0.30), (x * 2.9, 0.02, 2.20), "bark", rot_y=-58 if x < 0 else 58)
