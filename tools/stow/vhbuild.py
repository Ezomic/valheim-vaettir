"""
Shared model-building helpers, vendored.

The original lives in the vaettir repo, which is where it was written and where it is
maintained. This is a copy, because Stow ships as its own public repository and a
design script that imports from a sibling checkout is a script that does not run for
anyone who clones only this one.

Keep the two in step by copying, not by editing this in place - if a helper needs
changing, change vaettir/tools/vhbuild.py and copy it back over.
"""
